...
$('go').onclick=()=>{if(!$('text').value.trim()){notice('请输入朗读文字。');return}for(const k of keys)if(!$(k).checkValidity()){$('params-tab').click();$(k).reportValidity();return}remember();const data={text:$('text').value,reference:selectedRef.file,ref_text:$('transcript').value,model:$('model').value,settings:params()};action(async()=>{notice('任务已提交，运行进度显示在下方。');const h=await api('/generate',data);await library();selectResult(h);notice(`生成完成 · 推理 ${h.elapsed} s · 总耗时 ${h.total_elapsed} s${h.truncated?' · 达到上限，请检查结尾是否完整':''}`)})};
